...
class Document():
    ...
    def __init__(self, document_text=None, morph_path=None, document_path=None):
        """
        Parameters
        ----------
        document_text : str
        ...
        self.sentences: List[str] = sentencize(self.raw) if isinstance(self.raw, str) else [sentencize(text) for text in self.raw]
        self._index = 0
...
def sentencize(
    ...
    if len(list_of_string_sentences)==1:
        list_of_string_sentences = [
                sent\
                    for txt in list_of_string_sentences[0].split('\n')\
                        for sent in [' '.join(txt.strip().split()[:int(len(txt.strip().split())*i)]) if len(txt.strip().split())>6 else txt for i in [1/3,2/3,1]]]
    else:
        list_of_string_sentences = [
                sent\
                    for txt in list_of_string_sentences\
                        for sent in [' '.join(txt.strip().split()[:int(len(txt.strip().split())*i)]) if len(txt.strip().split())>6 else txt for i in [1/3,2/3,1]]]
