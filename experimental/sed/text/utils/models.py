...
def prepare_model(task:str, training=False):
    ...
    if "sent" in task:
        ...
        if not training:
            model.load_state_dict(torch.load(Path(ModelsConfig.lm_model_path)))
            model.eval()
    
            return model, train_vocab

        val_vocab = Vocab(document_path=ModelsConfig.lm_val_text, morpheme_template=ModelsConfig.morphs, label_encoder=train_vocab.label_encoder)
        ...
        if training:
            ...
            model = WordEmbeddings(text_data, embed_path=ModelsConfig.lm_embed_path, weight_path=ModelsConfig.lm_weight_path)
